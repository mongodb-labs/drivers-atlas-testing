...
ATLASORGANIZATIONNAME_OPTION = create_click_option(CONFIGOPTS.ATLAS_ORGANIZATION_NAME)

ATLASORGANIZATIONID_OPTION = create_click_option(CONFIGOPTS.ATLAS_ORGANIZATION_ID)
...
POLLINGFREQUENCY_OPTION = create_click_option(CONFIGOPTS.ATLAS_POLLING_FREQUENCY)
...
NODELETE_FLAG = click.option(
    ...
    default=False,
    help=(
        "Flag to instructs astrolabe to not delete clusters at the end of "
...
NOCREATE_FLAG = click.option(
    ...
    default=False,
    help=(
        "Do not create and configure clusters at the beginning of the run "
...
def cli(
    ...
    logging.basicConfig(
        level=loglevel,
        handlers=[ClickLogHandler()],
        format="%(levelname)s:%(name)s:%(message)s",
    )

    # Log atlasclient config.
    LOGGER.debug(tabulate_client_configuration(atlas_base_url, http_timeout))
...
def get_organization_by_id(ctx, org_id):
    ...
    pprint(cmd.get_organization_by_id(client=ctx.obj.client, org_id=org_id))
...
def create_project_if_necessary(
    ...
    org = cmd.get_organization_by_id(client=ctx.obj.client, org_id=org_id)
    pprint(
        cmd.ensure_project(
...
def resize_cluster(ctx, project_name, cluster_name, instance_size_name):
    ...
    cluster = (
        ctx.obj.client.groups[project.id]
        .clusters[cluster_name]
        .patch(**new_cluster_config)
    )
    pprint(cluster.data)
...
def isready_cluster(ctx, project_name, cluster_name):
    ...
    project = ctx.obj.client.groups.byName[project_name].get().data
    state = (
        ctx.obj.client.groups[project.id].clusters[cluster_name].get().data.stateName
    )
...
def validate_workload_executor(workload_executor, startup_time, connection_string):
    """
...
def get_logs_cmd(
    ...
    cluster_name = get_cluster_name(
        get_test_name(spec_test_file, workload_file), cluster_name_salt
    )

    organization = cmd.get_organization_by_id(client=ctx.obj.client, org_id=org_id)
    ...
    get_logs(
        admin_client=ctx.obj.admin_client, project=project, cluster_name=cluster_name
    )
...
def delete_test_cluster(
    ctx, spec_test_file, workload_file, org_id, project_name, cluster_name_salt
):
    """
    ...
    client = ctx.obj.client
    organization = cmd.get_organization_by_id(client=client, org_id=org_id)
...
@click.argument(
    "spec_test_file", type=click.Path(exists=True, dir_okay=False, resolve_path=True)
)
@WORKLOAD_FILE_OPTION
@WORKLOADEXECUTOR_OPTION
@CONNECTION_STRING_OPTION
...
def run_kubernetes_test(
    spec_test_file, workload_file, workload_executor, connection_string, xunit_output
):
    """
    ...
    xunit_logger.write_xml(test_case=xunit_test, filename=name)
