...
def tabulate_astrolabe_configuration(config):
    table_data = [
        ["Atlas organization name", config.organization_name],
    ...
    return table_txt.format(
        tabulate(table_data, headers=["Configuration option", "Value"], tablefmt="rst")
    )


def tabulate_client_configuration(base_url, http_timeout):
    table_data = [["Atlas API base URL", base_url], ["HTTP timeout (s)", http_timeout]]
